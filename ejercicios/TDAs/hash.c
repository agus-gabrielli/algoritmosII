...
{
    if (hash_overloaded(hash) && !hash_redimensionar(hash, hash->capacidad * FACTOR_REDIMENSION))
        return false;
...
void *hash_borrar(hash_t *hash, const char *clave)
{
    ...
    if (hash_underloaded(hash) && !hash_redimensionar(hash, hash->capacidad / FACTOR_REDIMENSION))
        return false;
...
{
    size_t pos = buscar_proximo_ocupado(iter, iter->pos_actual);
    return pos == iter->hash->capacidad;
}
...
bool hash_overloaded(hash_t *hash)
{
    return (hash->cantidad + hash->cantidad_borrados) / hash->capacidad >= FACTOR_CARGA;
}
